...
public struct EngineBindingGuard: Equatable, Sendable {
    ...
    /// True while the initial binding/start call is still on the stack. Godot
    /// startup may pump the main run loop, so SwiftUI can re-enter the view before
    /// `arm` is reached.
    public private(set) var isBinding = false
    ...
    /// Claim the right to perform the binding. A callback that re-enters while
    /// the claim is held must leave the in-flight engine start alone.
    public mutating func beginBinding(layer: ObjectIdentifier?, app: ObjectIdentifier?) -> Bool {
        guard needsBinding(layer: layer, app: app) else { return false }
        isBinding = true
        return true
    }
    ...
    /// Release an unsuccessful claim so a later update can retry startup.
    public mutating func cancelBinding() {
        isBinding = false
    }
