...
public class NSGodotAppView: GodotView {
            ...
            if instance.isStarted() {
                if app.displayDriver == "embedded" {
                    if !hasEmbeddedDisplayServer {
                        ...
                        } else if let displayServer = DisplayServer.shared as? DisplayServerEmbedded {
    ...
    func startGodotInstance() {
        ...
        if let app, let instance = app.instance {
            if app.displayDriver == "embedded" {
                guard renderingLayer != nil else {
                    Logger.App.error("startGodotInstance: renderingLayer was nil")
                    return
                }
            }
            let bindingLayer = layerIdentity
            let bindingApp = appIdentity
            guard bindingGuard.beginBinding(layer: bindingLayer, app: bindingApp) else { return }
            defer {
                if !bindingGuard.isArmed {
                    bindingGuard.cancelBinding()
                }
            }
            if app.displayDriver == "embedded", let renderingLayer {
                guard let rendererNativeSurface = RenderingNativeSurfaceApple.create(
                    layer: UInt(bitPattern: Unmanaged.passUnretained(renderingLayer).toOpaque())
                ) else {
                    Logger.App.error("startGodotInstance: could not create native rendering surface")
                    return
                }
                if MacEmbeddedDisplayServer.isAvailable {
                    MacEmbeddedDisplayServer.setNativeSurface(rendererNativeSurface)
                } else {
            ...
            if app.displayDriver == "embedded", !hasEmbeddedDisplayServer {
                if MacEmbeddedDisplayServer.isAvailable {
                    macEmbeddedIsReady = true
                } else if let displayServer = DisplayServer.shared as? DisplayServerEmbedded {
                    embedded = displayServer
            ...
            // caller tries again.
            if instance.isStarted() {
                bindingGuard.arm(layer: bindingLayer, app: bindingApp)
    ...
    private func enforceSurfaceSize() {
        guard app?.displayDriver == "embedded", hasEmbeddedDisplayServer else { return }
